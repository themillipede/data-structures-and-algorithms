...
# Process Pivot has been optimized to insert the value 0 in the pivot element column.
# With real number manipulation, this helps ensure within the tableau the reduction
# to zero is always clear and not approximated with the episilon functions.
...
# If the tableau provided by the first attempt leads to an invalid solution, meaning one of the inequality
# equations is violated with one of the values in the initial optimal values, then the algorithm will create
# a new tableau and proceed to a two-phase simplex method approach.
    ...
    while (phase_one_optimization or
        ...
        if phase_one_optimization and all(epsilon_less_than_equal_to(k, 0) for k in phase_one_row[:-1]):
            ...
            if all(epsilon_greater_than_equal_to(i, 0) for i in tableau[len(tableau)-1][:-1]):
                break
        no_solution, pivot_element = select_pivot_element(tableau, m, slack_rows, phase_one_optimization, phase_one_row)
        if no_solution:
        ...
        tableau, phase_one_row = process_pivot_element(tableau, pivot_element, phase_one_optimization, phase_one_row)
